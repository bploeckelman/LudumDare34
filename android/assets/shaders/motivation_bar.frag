...
//const float c_border_width = 0.01;
//const vec4 c_border_color = vec4(0,0,1,1);
...
void main()
{
    ...
    if (distance_from_target <= u_target_falloff_width) {
        if (distance_from_target == 0.) {
            color = vec4(0,1,0,1);
//            if (min(v_texCoords.x - u_target_x, (u_target_x + u_target_width) - v_texCoords.x) <= c_border_width) {
...
//            color = color * a;
//            color = vec4(0,0.8,0,1) * (1. - (distance_from_target / u_target_falloff_width));
        }
    }
